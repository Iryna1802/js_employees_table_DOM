...
for(let i = 0; i < titleTable.length; i++){
    ...
    if (titleTable[i].innerText === "Office") {
        ...
        columns.forEach((element)=>{
            if (!list.includes(element.cells[i].innerHTML)) list.push(element.cells[i].innerHTML);
        });
        list.forEach((name1)=>{
    ...
    if (input.name === "name" || input.name === "position") input.type = "text";
...
button.addEventListener("click", (e)=>{
    e.preventDefault();
    if (form.elements.name.value.trim().length >= 4 && form.elements.position.value.trim().length >= 4 && parseInt(form.elements.age.value) >= 18 && parseInt(form.elements.age.value) <= 90) {
        const row = bodyTable.insertRow();
        for(let i = 0; i < bodyTable.rows[0].cells.length; i++){
            row.insertCell();
            if (form.elements[i].name === "salary") {
                row.cells[i].textContent = `$${Number(form.elements[i].value).toLocaleString("en-US")}`;
                continue;
            }
            row.cells[i].textContent = form.elements[i].value;
        }
        pushNotification("Title of Success message", "Notification should contain title and description.", "success");
    } else pushNotification("Error!", "Notification should contain title and description.", "error");
});
...
titleContainer.addEventListener("click", (e)=>{
    ...
    if (index === 0 || index === 1 || index === 2) {
        ...
        if (target.classList.contains("asc")) {
            const sortNameDESC = columns.sort((a, b)=>{
                return b.cells[index].innerHTML.toLowerCase().localeCompare(a.cells[index].innerHTML.toLowerCase());
            });
            target.classList.toggle("asc");
            table.tBodies[0].append(...sortNameDESC);
        } else {
            const sortName = columns.sort((a, b)=>{
                return a.cells[index].innerHTML.toLowerCase().localeCompare(b.cells[index].innerHTML.toLowerCase());
            });
            target.classList.toggle("asc");
    ...
    if (index === 3) {
        deleteClass(titleContainer.cells, index);
        if (target.classList.contains("asc")) {
            const sortAgeDesk = columns.sort((rowA, rowB)=>rowB.cells[index].innerHTML - rowA.cells[index].innerHTML);
            table.tBodies[0].append(...sortAgeDesk);
            target.classList.toggle("asc");
        } else {
            const sortAge = columns.sort((rowA, rowB)=>rowA.cells[3].innerHTML - rowB.cells[3].innerHTML);
    ...
    if (index === 4) {
        deleteClass(titleContainer.cells, index);
        if (target.classList.contains("asc")) {
            const sortNumDesk = columns.sort((a, b)=>getNumber(b.cells[index].innerHTML) - getNumber(a.cells[index].innerHTML));
            target.classList.toggle("asc");
            table.tBodies[0].append(...sortNumDesk);
        } else {
            const sortSalary = columns.sort((a, b)=>getNumber(a.cells[index].innerHTML) - getNumber(b.cells[index].innerHTML));
            target.classList.toggle("asc");
...
function deleteClass(array, obj) {
    for(let i = 0; i < array.length; i++){
        if (i === obj) continue;
        if (array[i].classList.contains("asc")) array[i].classList.toggle("asc");
    }
}
...
bodyTable.addEventListener("click", (e)=>{
    ...
    if (!bodyTable.contains(target)) return;
    if (!target) return;
